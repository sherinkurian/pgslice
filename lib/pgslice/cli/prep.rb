  ...
  class CLI
    ...
    def prep(table, column=nil, period=nil)
      ...
      if declarative && options[:partition]
        queries << <<-SQL
CREATE TABLE #{quote_table(intermediate_table)} (LIKE #{quote_table(table)} INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING STORAGE INCLUDING COMMENTS INCLUDING INDEXES) PARTITION BY RANGE (#{quote_ident(column)});
        SQL
